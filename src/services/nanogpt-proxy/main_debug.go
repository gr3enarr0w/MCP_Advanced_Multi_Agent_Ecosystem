...
func mainDebug() {
	...
	log.Printf("[DEBUG] Initializing %d MCP clients...", len(cfg.MCPServers))
	mcpClients := make(map[string]*mcp.MCPClient)
	for serverName, serverCfg := range cfg.MCPServers {
		log.Printf("[DEBUG] Creating MCP client for '%s' with command: %s %v", serverName, serverCfg.Command, serverCfg.Args)
		...
		go func(name string, c *mcp.MCPClient) {
			log.Printf("[DEBUG] Starting connection to MCP server '%s'...", name)
	...
	log.Printf("[DEBUG] MCP client initialization completed, waiting for connections...")
